...
public class ProjectileSystem {
    ...
    static class Projectile {
        ...
        int trailCursor = 0;
        int trailFrameSkip = 0;

        void init(float x, float y, float velX, float velY,
                  int damage, int color, float w, float h, int type) {
            this.active = true;
            this.x = x; this.y = y;
    ...
    final float[] flashX = new float[MAX_FLASHES];
    final float[] flashY = new float[MAX_FLASHES];
    final float[] flashLife = new float[MAX_FLASHES]; 
    final int[] flashColor = new int[MAX_FLASHES];
    int flashCursor = 0;

    private float screenWidth, screenHeight;

    private final Paint projPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
    ...
    public float autoFire(float dt, float shipX, float shipY, float bankAngle, ShipData ship) {
        ...
        if (fireTimer >= fireInterval) {
            ...
            float baseSpeed = -800f * ship.projectileSpeed; 
            ...
            for (int w = 0; w < ship.projectileCount; w++) {
                ...
                float localX = ship.weaponX[w];
                float localY = ship.weaponY[w];
                float worldX = shipX + localX * cos - localY * sin;
                float worldY = shipY + localX * sin + localY * cos;

                float pw, ph;
                switch (ship.id) {
                    case ShipRegistry.JUGGERNAUT: pw = 4f; ph = 10f; break;
                    case ShipRegistry.PHANTOM: pw = 2f; ph = 14f; break;
                    default: pw = 2.5f; ph = 8f; break;
                }

                Projectile p = pool[cursor];
                cursor = (cursor + 1) % POOL_SIZE;
                p.init(worldX, worldY, 0, baseSpeed, ship.damage, ship.projectileColor, pw, ph, ship.id);

                flashX[flashCursor] = worldX;
                flashY[flashCursor] = worldY;
                flashLife[flashCursor] = 1f;
                flashColor[flashCursor] = ship.projectileColor;
    ...
    public void update(float dt) {
        ...
        for (int i = 0; i < MAX_FLASHES; i++) {
            if (flashLife[i] > 0) flashLife[i] -= dt * 12f; 
        }
    }
    ...
    private void drawProjectile(Canvas canvas, Projectile p) {
        ...
        switch (p.shipType) {
            case ShipRegistry.STRIKER:
                projPath.reset();
                projPath.moveTo(p.x, p.y - p.height / 2);
                projPath.lineTo(p.x + p.width / 2, p.y);
                projPath.lineTo(p.x, p.y + p.height / 2);
                projPath.lineTo(p.x - p.width / 2, p.y);
                projPath.close();
                canvas.drawPath(projPath, projPaint);
                projPaint.setColor(Color.argb(200, 255, 255, 255));
                canvas.drawCircle(p.x, p.y, 1.5f, projPaint);
                break;

            case ShipRegistry.JUGGERNAUT:
                canvas.drawRoundRect(p.x - p.width / 2, p.y - p.height / 2, p.x + p.width / 2, p.y + p.height / 2, 1.5f, 1.5f, projPaint);
                projPaint.setColor(Color.argb(220, 255, 230, 180));
                canvas.drawRect(p.x - p.width / 2 + 0.5f, p.y - p.height / 2, p.x + p.width / 2 - 0.5f, p.y - p.height / 2 + 3, projPaint);
                break;
            ...
            case ShipRegistry.PHANTOM:
                ...
                projPaint.setColor(Color.argb(40, Color.red(p.color), Color.green(p.color), Color.blue(p.color)));
                canvas.drawCircle(p.x, p.y, p.height * 0.4f, projPaint);
                break;
        }
    }
    ...
    private void drawTrail(Canvas canvas, Projectile p) {
        ...
        for (int t = 0; t < Projectile.TRAIL_LENGTH - 1; t++) {
            int idx0 = (p.trailCursor + t) % Projectile.TRAIL_LENGTH;
            int idx1 = (p.trailCursor + t + 1) % Projectile.TRAIL_LENGTH;
            float age = (float) t / Projectile.TRAIL_LENGTH;
            int alpha = (int)(80 * (1f - age));
            if (alpha < 5) continue;
    ...
    private void drawMuzzleFlashes(Canvas canvas) {
        for (int i = 0; i < MAX_FLASHES; i++) {
            ...
            float life = flashLife[i];
            float radius = 5f + (1f - life) * 4f; 
            ...
            flashPaint.setColor(Color.argb((int)(200 * life), 255, 255, 255));
            canvas.drawCircle(flashX[i], flashY[i], radius * 0.6f, flashPaint);
        }
    }

    // Callbacks for collision actions
        ...
        for (int pi = 0; pi < POOL_SIZE; pi++) {
            ...
            for (Asteroid a : asteroids) {
                ...
                if (distSq < hitDist * hitDist) {
                    p.active = false;
                    boolean destroyed = a.takeDamage(p.damage, economy);
                    anyHit = true;
                    if (destroyed) {
                        callback.onAsteroidDestroyed(a);
                    } else {
                        callback.onAsteroidHit(a);
                    }
                    break;
                }
            }
