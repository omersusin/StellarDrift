...
public class GameWorld {
    ...
    private int screenW, screenH, state, score, frameCount;
    ...
    private List<StarDust> starDusts = new ArrayList<>();
    private List<float[]> pendingSpawns = new ArrayList<>(); // {x, radius, timer}
    private List<Particle> particles = new ArrayList<>();
    private List<PowerUp> powerUps = new ArrayList<>();
    private List<ScorePopup> popups = new ArrayList<>();
    private List<float[]> spawnWarnings = new ArrayList<>();
    private List<float[]> nearMissFlashes = new ArrayList<>();
    private List<float[]> ringBursts = new ArrayList<>();

    private SettingsManager settings;
    private SoundManager sound;
    private VibrationManager vibration;

    private int combo, comboTimer, nearMissCooldown, magnetTimer, slowmoTimer, doubleTimer, powerUpSpawnTimer;
    private boolean magnetActive, slowmoActive;
    
    // HATA 2 ÇÖZÜMÜ: Eksik Değişkenler Eklendi
    ...
    private int tempoPhase, tempoTimer, riskWindowTimer, freezeTimer;
    private boolean riskWindowActive, overdriveTriggered, shockwaveActive;
    private Asteroid killerAsteroid;
    private float shockwaveX, shockwaveY, shockwaveRadius, shockwaveAlpha;
    private int lastMilestone, milestoneTimer;
    private String milestoneText;
    private float dangerLevel;
    private float[] directionalDangers = new float[4];

    private int grazeChainCount, grazeChainTimer, chainCounter, chainTarget;
    private boolean chainActive, firstStarDustSeen, firstNearMiss;
    ...
    public void setAudioEngine(SoundManager sm, VibrationManager vm) { this.sound = sm; this.vibration = vm; }
    public void setSpawnIndicator(SpawnIndicator si) { this.spawnIndicator = si; }
    ...
    public GameWorld(int sw, int sh, Context ctx) {
        ...
        state = Constants.STATE_MENU; tempoPhase = Constants.TEMPO_CALM; tempoTimer = Constants.TEMPO_CALM_DURATION;
    }
    ...
    public void update(float joyDirX, float joyDirY, float joyMag) {
        ...
        sessionStats.minFuelReached = Math.min(sessionStats.minFuelReached, fuelSystem.getFuelRatio() * 100f);
        fuelSystem.setDrainPaused(isGodMode); fuelSystem.update(dt);
        hitStallTimer = Math.max(0, hitStallTimer - dt);
        float effectiveDtScale = (hitStallTimer > 0) ? Constants.HIT_STALL_TIME_SCALE : 1f;
        float speedMult = settings.getSpeedMultiplier(), gameSpeedMult = settings.getGameSpeedMultiplier();
        ...
        player.setComboInfo(combo, comboTimer > 0 ? (float)comboTimer / Constants.COMBO_TIMEOUT : 0f);
        updateSpawnBias(dt, joyDirX, joyDirY, joyMag);
        
        ShipData currentShip = shipRegistry.getSelectedShip();
        float recoil = projectiles.autoFire(dt, player.getX(), player.getY(), player.getBankAngle(), currentShip, plasmaCore.getFireRateMultiplier());
        ...
        projectiles.update(dt); plasmaCore.update(dt);

        if (hitStallTimer <= 0) { updateTimers(dt); updateTempo(); handleSpawning(); updateSpawnWarnings(); updatePendingSpawns(); }

        updateDirectionalDangers(dt);
        moveEntities(getEffectiveDifficulty(speedMult, gameSpeedMult) * effectiveDtScale);
        applyStarDustMagnet();
    ...
    private void updateTimers(float dt) {
        ...
        if (magnetActive) { magnetTimer--; if (magnetTimer <= 0) magnetActive = false; } if (slowmoActive) { slowmoTimer--; if (slowmoTimer <= 0) slowmoActive = false; }
        if (doubleActive) { doubleTimer--; if (doubleTimer <= 0) { economy.setDoubleActive(false); doubleActive = false; } } 
    ...
    private void handleSpawning() {
        ...
        if (!warmup && !isGodMode) {
            ...
            if (frameCount % astInt == 0) {
                ...
                float radius = screenW * (Constants.ASTEROID_MIN_SIZE + (float)(Math.random() * range));
                float px = player.getX();
                float safeZone = screenW * Constants.ASTEROID_SAFE_ZONE;
                float tryX; int attempts = 0;
                do { tryX = (float)(Math.random() * (screenW - radius * 2) + radius); attempts++; } 
                while (Math.abs(tryX - px) < safeZone && attempts < 10);
                
                if (spawnIndicator != null) spawnIndicator.showIncoming(tryX, radius);
                pendingSpawns.add(new float[]{tryX, radius, 50}); 
            }
        }
        ...
        if (frameCount % sdInt == 0) { if (Math.random() < 0.15f) { plasmaCore.trySpawn((float)(Math.random() * screenW), -50, Constants.STARDUST_SPEED * (screenW / 1080f)); } else if (Math.random() < Constants.STARDUST_CHAIN_CHANCE && !chainActive) { spawnStarDustChain(); } else { starDusts.add(spawnBiasedStarDust()); } }
    ...
    private StarDust spawnBiasedStarDust() { StarDust sd = new StarDust(screenW, screenH); if (Math.random() <= Constants.SPAWN_BIAS_STRENGTH) { float aheadDist = 150f + (float)(Math.random() * 150f); float angleOffset = (float)((Math.random() - 0.5) * Math.toRadians(60)); float finalAngle = spawnBiasAngle + angleOffset; float spawnX = player.getX() + (float) Math.cos(finalAngle) * aheadDist; spawnX = Math.max(20, Math.min(screenW - 20, spawnX)); sd.setPosition(spawnX, sd.getY()); } return sd; }
    private void spawnStarDustChain() { int count = Constants.STARDUST_CHAIN_MIN + (int)(Math.random() * (Constants.STARDUST_CHAIN_MAX - Constants.STARDUST_CHAIN_MIN + 1)); if (isGodMode) count += 3; chainTarget = count; chainCounter = 0; chainActive = true; float startX = spawnBiasedStarDust().getX(); float endX = spawnBiasedStarDust().getX(); for (int i = 0; i < count; i++) { float t = (float) i / (count - 1), cx = startX + (endX - startX) * t, cy = -(i * 100 + 50), wobble = (float)(Math.sin(i * 1.2) * screenW * 0.04); StarDust sd = new StarDust(screenW, screenH); sd.setPosition(cx + wobble, cy); starDusts.add(sd); } }
    ...
    private void updatePendingSpawns() { Iterator<float[]> it = pendingSpawns.iterator(); while (it.hasNext()) { float[] p = it.next(); p[2]--; if (p[2] <= 0) { asteroids.add(new Asteroid(screenW, screenH, p[0], p[1])); it.remove(); } } }
    ...
    private void checkCollisions() {
        ...
        if (plasmaCore.checkCollection(px, py, player.getCollisionRadius())) { sessionStats.plasmaCoresCollected++; spawnCollectBurst(px, py, Color.rgb(60, 180, 255)); popups.add(new ScorePopup(px, py - 30, "⚡ OVERCHARGE!", Color.rgb(80, 200, 255), 1.5f)); shakeIntensity += 3f; score += 50; if(vibration!=null) vibration.vibrateOvercharge(); if(sound!=null) sound.playOvercharge(); }

        projectiles.checkHits(asteroids, economy, new ProjectileSystem.HitCallback() {
            @Override public void onAsteroidHit(Asteroid asteroid) { sessionStats.shotsHit++; spawnParticles(asteroid.getX(), asteroid.getY(), 3, Color.WHITE); if(sound!=null) sound.playHit(); if(vibration!=null) vibration.vibrateHit(); }
            @Override public void onAsteroidDestroyed(Asteroid asteroid) { sessionStats.shotsHit++; sessionStats.asteroidsDestroyed++; spawnParticles(asteroid.getX(), asteroid.getY(), 15, Color.rgb(200, 180, 100)); spawnRingBurst(asteroid.getX(), asteroid.getY(), Color.rgb(255, 215, 0)); int cred = economy.isDoubleActive() ? asteroid.getCreditValue() * 2 : asteroid.getCreditValue(); popups.add(new ScorePopup(asteroid.getX(), asteroid.getY(), "+ " + cred + " ✦", Color.rgb(255, 215, 0), economy.isDoubleActive() ? 1.5f : 1.2f)); if(sound!=null) sound.playExplode(); if(vibration!=null) vibration.vibrateExplode(); score += 25; }
        });
    ...
    private void onNearMiss(float px, float py, float ax, float ay) { sessionStats.nearMisses++; nearMissCooldown = Constants.NEAR_MISS_COOLDOWN; nearMissCount++; if (!firstNearMiss) firstNearMiss = true; grazeChainCount++; grazeChainTimer = Constants.GRAZE_CHAIN_WINDOW; int grazePoints = Constants.NEAR_MISS_BONUS * (1 << (Math.min(grazeChainCount, 6) - 1)); if (riskWindowActive) grazePoints = (int)(grazePoints * Constants.RISK_WINDOW_MULT); score += grazePoints; int textColor; switch (grazeChainCount) { case 1: textColor = Color.WHITE; break; case 2: textColor = Color.CYAN; break; case 3: textColor = Color.MAGENTA; break; default: textColor = Color.rgb(255, 100, 0); break; } if (grazeChainCount >= 3) popups.add(new ScorePopup(px, py - 70, grazeChainCount >= 5 ? "INSANE!" : "DAREDEVIL!", textColor, 1.6f)); popups.add(new ScorePopup(px, py - 40, "+" + grazePoints, textColor, 1f)); riskWindowActive = true; riskWindowTimer = Constants.RISK_WINDOW_DURATION; nearMissFlashes.add(new float[]{px, py, ax, ay, Constants.NEAR_MISS_FLASH_LIFE}); if(sound!=null) sound.playNearMiss(); if (grazeChainCount > 1 && vibration!=null) vibration.vibrateNearMiss(); }
    ...
    private void triggerGameOver() {
        ...
        economy.convertSessionScore(score);
        
        sessionStats.score = score;
        sessionStats.maxCombo = maxCombo;
        sessionStats.shotsFired = projectiles.totalShotsFired;
        sessionStats.creditsEarned = score / 10;
        
        if(sound!=null) sound.playGameOver();
        isGodMode = false; soundToggleClicks = 0; vibToggleClicks = 0;
    }
    ...
    // HATA 5 ÇÖZÜMÜ: handleTap() eklendi
    ...
    public void startGame() {
        ...
        fuelSystem.reset(); startTime = System.currentTimeMillis(); asteroids.clear(); starDusts.clear(); particles.clear(); powerUps.clear(); popups.clear(); spawnWarnings.clear(); pendingSpawns.clear(); nearMissFlashes.clear(); ringBursts.clear(); plasmaCore.reset(); player.reset(); projectiles.reset();
    ...
    public int getState() { return state; } public void setState(int s) { this.state = s; } 
