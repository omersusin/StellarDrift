...
public class GameWorld {
    ...
    // HATA ÇÖZÜMÜ: FuelSystem referansı tutulmalı
    ...
    public GameWorld(int sw, int sh, Context ctx) {
        ...
        economy = new EconomyManager(ctx);
        
        // HATA ÇÖZÜMÜ: Player'ı çağırmadan önce FuelSystem'i oluştur
        fuelSystem = new FuelSystem(sw, sh);
        
        // HATA ÇÖZÜMÜ: Player artık 4 parametre alıyor
    ...
    public void update(float joyDirX, float joyDirY, float joyMag) {
        ...
        // HATA ÇÖZÜMÜ: FuelSystem'i de update et
    ...
    private void updateTimers() {
        ...
        if (doubleActive) { doubleTimer--; if (doubleTimer <= 0) doubleActive = false; } if (riskWindowActive) { riskWindowTimer--; if (riskWindowTimer <= 0) riskWindowActive = false; }
    ...
    private void handleSpawning() {
        ...
        if (frameCount % sdInt == 0) { if (Math.random() < Constants.STARDUST_CHAIN_CHANCE && !chainActive) spawnStarDustChain(); else starDusts.add(spawnBiasedStarDust()); }
    ...
    private void triggerGameOver() { state = Constants.STATE_GAME_OVER; settings.setHighScore(score); settings.incrementGamesPlayed(); economy.convertSessionScore(score); sound.playGameOver(); isGodMode = false; soundToggleClicks = 0; vibToggleClicks = 0; }
    ...
    public void toggleSound() { settings.toggleSound(); sound.setEnabled(settings.isSoundEnabled()); if (settings.isSoundEnabled()) sound.playClick(); if (!isGodMode) { soundToggleClicks++; if (soundToggleClicks >= 10) { isGodMode = true; vibration.vibrateExplosion(); } } }
    ...
    // EKSIK OLAN 2 GETTER BURADA:
    public FuelSystem getFuelSystem() { return fuelSystem; }
    public boolean isGodModeActive() { return isGodMode; }
}
