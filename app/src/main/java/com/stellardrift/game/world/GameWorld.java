...
public class GameWorld {
    ...
    private void checkCollisions() {
        ...
        if (plasmaCore.checkCollection(px, py, player.getCollisionRadius())) { spawnCollectBurst(px, py, Color.rgb(60, 180, 255)); popups.add(new ScorePopup(px, py - 30, "⚡ OVERCHARGE!", Color.rgb(80, 200, 255), 1.5f)); shakeIntensity += 3f; score += 50; if(vibration!=null) vibration.vibrateOvercharge(); if(sound!=null) sound.playOvercharge(); }
        ...
        projectiles.checkHits(asteroids, economy, new ProjectileSystem.HitCallback() {
            ...
            @Override public void onAsteroidDestroyed(Asteroid asteroid) { spawnParticles(asteroid.getX(), asteroid.getY(), 15, Color.rgb(200, 180, 100)); spawnRingBurst(asteroid.getX(), asteroid.getY(), Color.rgb(255, 215, 0)); int cred = economy.isDoubleActive() ? asteroid.getCreditValue() * 2 : asteroid.getCreditValue(); popups.add(new ScorePopup(asteroid.getX(), asteroid.getY(), "+ " + cred + " ✦", Color.rgb(255, 215, 0), economy.isDoubleActive() ? 1.5f : 1.2f)); if(sound!=null) sound.playExplode(); if(vibration!=null) vibration.vibrateExplode(); score += 25; }
        });
    ...
    private void triggerGameOver() { state = Constants.STATE_GAME_OVER; settings.setHighScore(score); settings.incrementGamesPlayed(); economy.convertSessionScore(score); if(sound!=null) sound.playGameOver(); isGodMode = false; soundToggleClicks = 0; vibToggleClicks = 0; }
