...
public class StarDust {
    ...
    private float pulsePhase, rotation, rotSpeed;
    private float pullVX, pullVY;
    ...
    public StarDust(int sw, int sh) {
        ...
        speed = Constants.STARDUST_SPEED * (sw / 1080f);
        size = sw * Constants.STARDUST_SIZE_RATIO;
        pulsePhase = (float)(Math.random() * Math.PI * 2);
        rotation = 0;
        rotSpeed = (float)(Math.random() * 3 - 1.5);
        pullVX = 0; pullVY = 0;

        glowPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        glowPaint.setColor(GOLD); glowPaint.setStyle(Paint.Style.FILL);
        corePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        corePaint.setColor(GOLD); corePaint.setStyle(Paint.Style.FILL);
        centerPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        centerPaint.setStyle(Paint.Style.FILL);
        rayPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        rayPaint.setColor(GOLD); rayPaint.setStrokeCap(Paint.Cap.ROUND);
        boundsRect = new RectF();
    }

    public void update(float difficulty) {
        y += speed * difficulty + pullVY;
        x += pullVX;
        pullVX *= 0.85f;
        pullVY *= 0.85f;
        pulsePhase += 0.08f;
    ...
    public void pull(float vx, float vy) {
        pullVX += vx;
        pullVY += vy;
    }
    ...
    public void render(Canvas c) {
        ...
        rayPaint.setAlpha(100);
        rayPaint.setStrokeWidth(size * 0.12f);
        for (int i = 0; i < 4; i++) {
        ...
        corePaint.setAlpha(255);
        c.drawCircle(x, y, ds, corePaint);
        centerPaint.setColor(GOLD_LIGHT); centerPaint.setAlpha(220);
        c.drawCircle(x, y, ds * 0.5f, centerPaint);
        centerPaint.setColor(Color.WHITE); centerPaint.setAlpha(200);
        c.drawCircle(x, y, ds * 0.22f, centerPaint);
    }
    ...
    public RectF getBounds() {
        boundsRect.set(x - size, y - size, x + size, y + size);
        return boundsRect;
    }
}
