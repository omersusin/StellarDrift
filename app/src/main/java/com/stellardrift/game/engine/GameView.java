...
public class GameView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        if (background == null)
            background = new SpaceBackground(screenW, screenH);
        if (gameWorld == null)
            gameWorld = new GameWorld(screenW, screenH, getContext());
        if (renderer == null)
    ...
    public void resume() { /* surfaceCreated handles restart */ }
    ...
    private void handleTap(float x, float y) {
        ...
        if (state == Constants.STATE_MENU) {
            if (uiOverlay.isPlayHit(x, y)) {
                gameWorld.startGame();
            } else if (uiOverlay.isSettingsHit(x, y)) {
                gameWorld.openSettings();
            }
        } else if (state == Constants.STATE_SETTINGS) {
            if (uiOverlay.isDiffHit(x, y)) {
                gameWorld.cycleDifficulty();
            } else if (uiOverlay.isSoundHit(x, y)) {
                gameWorld.toggleSound();
            } else if (uiOverlay.isVibHit(x, y)) {
                gameWorld.toggleVibration();
            } else if (uiOverlay.isBackHit(x, y)) {
                gameWorld.closeSettings();
            }
    ...
    public void drawGame(Canvas canvas) {
        ...
        if (renderer != null && gameWorld != null)
            renderer.render(canvas, gameWorld);
        if (uiOverlay != null && gameWorld != null)
            uiOverlay.render(canvas, gameWorld.getState(),
                gameWorld.getScore(), gameWorld.getHighScore(),
                gameWorld.getDifficulty(), gameWorld.getSettings());
    }
}
