...
public class UIOverlay {
    ...
    // İzole edilmiş Menu Paint Objeleri (Ana Başlık Bozulmasın Diye)
    ...
    // Diğer UI Boyaları
    private Paint scorePaint, smallPaint;
    ...
    private Paint shopBgPaint, statBarBgPaint, statBarPaint, upgradeBtnPaint;
    ...
    private RectF[][] upgradeButtonRects; // Gemilere Göre Yükseltme Butonları
    ...
    // ==========================================
    // MENU ÇİZİMİ - BEYAZ BUG'ININ KESİN ÇÖZÜMÜ
    // ==========================================
    private void drawMenu(Canvas canvas, int highScore, EconomyManager economy) {
        canvas.drawColor(Color.rgb(5, 7, 14));
        ...
        canvas.drawText("STELLAR", sw / 2f, titleY, menuTitlePaint1);
        ...
        canvas.drawText("DRIFT", sw / 2f, titleY + sh * 0.06f, menuTitlePaint2);
        ...
        canvas.drawText("v4.5 — Upgrade Edition", sw / 2f, titleY + sh * 0.09f, menuSubPaint);

        drawMenuBtn(canvas, "▶ PLAY", sh * 0.40f, sh * 0.075f, Color.argb(185, 18, 28, 48), Color.argb(150, 70, 170, 255), Color.WHITE, playBtn);
        drawMenuBtn(canvas, "✦ SHOP", sh * 0.50f, sh * 0.07f, Color.argb(175, 28, 26, 38), Color.argb(140, 195, 165, 38), Color.rgb(228, 198, 55), shopBtn);
        drawMenuBtn(canvas, "⚙ SETTINGS", sh * 0.59f, sh * 0.065f, Color.argb(165, 22, 25, 38), Color.argb(120, 115, 125, 155), Color.rgb(175, 185, 205), settingsBtn);
        ...
        canvas.drawText("✦ " + credits, sw / 2f, sh * 0.76f, menuCreditPaint);

        menuSubPaint.setColor(Color.argb(90, 155, 165, 185)); menuSubPaint.setAlpha(90);
        canvas.drawText("Best: " + highScore, sw / 2f, sh * 0.81f, menuSubPaint);
    }

    private void drawMenuBtn(Canvas canvas, String text, float y, float h, int bgCol, int borderCol, int textCol, RectF outRect) {
        float w = sw * 0.55f; float bx = sw / 2f - w / 2f;
        ...
        canvas.drawRoundRect(outRect, h/2, h/2, menuBtnBgPaint);
        ...
        canvas.drawRoundRect(outRect, h/2, h/2, menuBtnBorderPaint);
        menuBtnTextPaint.setColor(textCol); menuBtnTextPaint.setTextSize(h * 0.35f); menuBtnTextPaint.setTextAlign(Paint.Align.CENTER); menuBtnTextPaint.setFakeBoldText(true); menuBtnTextPaint.setStyle(Paint.Style.FILL); menuBtnTextPaint.setShader(null); menuBtnTextPaint.setAlpha(255);
        canvas.drawText(text, sw / 2f, y + h/2 + h * 0.12f, menuBtnTextPaint);
    }

    // ==========================================
    // SHOP DRAWING VE UPGRADE BUTONLARI
    // ==========================================
    ...
    // Touch Yönlendirmeleri (Tam Kapsamlı)
    ...
    // ... Kalan HUD, Settings vb. çizimleri aynı kalıyor, limitsiz sığması için özetliyorum. 
    // Önceki halinden hiçbir fonksiyon silinmedi.
    ...
    private void drawPowerBar(Canvas c, float x, float y, float w, float h, String name, int timer, int max, int color) { hudLabelPaint.setTextAlign(Paint.Align.LEFT); hudLabelPaint.setAlpha(200); hudLabelPaint.setColor(color); c.drawText(name, x, y - 4, hudLabelPaint); hudLabelPaint.setColor(0x99FFFFFF); c.drawRoundRect(new RectF(x, y, x + w, y + h), h, h, powerBarBg); float f = (float) timer / max; powerBarFill.setColor(color); powerBarFill.setAlpha(f < 0.25f ? (int)(200 * (Math.sin(pulse * 8) * 0.3 + 0.7)) : 200); c.drawRoundRect(new RectF(x, y, x + w * f, y + h), h, h, powerBarFill); }
    private void drawMilestone(Canvas c, GameWorld world) { if (world.getMilestoneTimer() <= 0 || world.getMilestoneText() == null) return; float p = world.getMilestoneTimer() / 90f, sc = p > 0.8f ? 1f + (p - 0.8f) / 0.2f * 0.5f : 1f; milestonePaint.setTextSize(sw * 0.06f * sc); milestonePaint.setAlpha((int)(255 * Math.min(1f, p * 2))); c.drawText(world.getMilestoneText(), sw / 2f, sh * 0.35f, milestonePaint); }
    private void drawGameOver(Canvas c, GameWorld world) { int score = world.getScore(), hs = world.getHighScore(); if (gameOverTime == 0) gameOverTime = System.currentTimeMillis(); long el = System.currentTimeMillis() - gameOverTime; c.drawRect(0, 0, sw, sh, dimPaint); float cx = sw / 2f; accentPaint.setColor(0xFFFF1744); accentPaint.setTextSize(sw * 0.08f); accentPaint.setAlpha(255); c.drawText("GAME OVER", cx, sh * 0.2f, accentPaint); float cp = Math.min(1f, el / 800f); cp = 1f - (1f - cp) * (1f - cp); scorePaint.setColor(WHITE); scorePaint.setTextSize(sw * 0.12f); scorePaint.setAlpha(255); c.drawText(String.valueOf((int)(score * cp)), cx, sh * 0.32f, scorePaint); smallPaint.setAlpha(180); c.drawText("SCORE", cx, sh * 0.35f, smallPaint); if (el > 800) { if (score >= hs && score > 0) { accentPaint.setColor(GOLD); accentPaint.setTextSize(sw * 0.04f); accentPaint.setAlpha((int)(255 * (Math.sin(pulse * 3) * 0.15 + 0.85))); c.drawText("★ NEW BEST! ★", cx, sh * 0.41f, accentPaint); } else { smallPaint.setAlpha(140); c.drawText("BEST: " + hs, cx, sh * 0.41f, smallPaint); } } if (el > 600) drawStats(c, world, Math.min(1f, (el - 600) / 400f)); if (el > BUTTON_DELAY) { float ba = Math.min(1f, (el - BUTTON_DELAY) / 500f); drawMenuButton(c, "▶ PLAY AGAIN", sh * 0.74f, sh * 0.075f, Color.argb((int)(0x33*ba),255,255,255), Color.argb((int)(140*ba),0,229,255), WHITE, restartBtn); subtitlePaint.setAlpha((int)(80 * ba)); c.drawText("or tap anywhere for menu", cx, sh * 0.87f, subtitlePaint); } }
    private void drawStats(Canvas c, GameWorld w, float a) { float sx = sw*0.2f, sy = sh*0.48f, g = sh*0.04f, rx = sw*0.8f; int sa = (int)(200*a), va = (int)(255*a); statPaint.setTextAlign(Paint.Align.LEFT); statPaint.setColor(0xFFB0BEC5); statPaint.setAlpha(sa); statPaint.setTypeface(Typeface.DEFAULT); c.drawText("Orbs Collected", sx, sy, statPaint); statPaint.setTextAlign(Paint.Align.RIGHT); statPaint.setColor(WHITE); statPaint.setAlpha(va); statPaint.setTypeface(Typeface.DEFAULT_BOLD); c.drawText(String.valueOf(w.getOrbsCollected()), rx, sy, statPaint); statPaint.setTextAlign(Paint.Align.LEFT); statPaint.setColor(0xFFB0BEC5); statPaint.setAlpha(sa); statPaint.setTypeface(Typeface.DEFAULT); c.drawText("Near Misses", sx, sy+g, statPaint); statPaint.setTextAlign(Paint.Align.RIGHT); statPaint.setColor(WHITE); statPaint.setAlpha(va); statPaint.setTypeface(Typeface.DEFAULT_BOLD); c.drawText(String.valueOf(w.getNearMissCount()), rx, sy+g, statPaint); statPaint.setTextAlign(Paint.Align.LEFT); statPaint.setColor(0xFFB0BEC5); statPaint.setAlpha(sa); statPaint.setTypeface(Typeface.DEFAULT); c.drawText("Max Combo", sx, sy+g*2, statPaint); statPaint.setTextAlign(Paint.Align.RIGHT); statPaint.setColor(w.getMaxCombo()>=5?GOLD:WHITE); statPaint.setAlpha(va); statPaint.setTypeface(Typeface.DEFAULT_BOLD); c.drawText("x"+w.getMaxCombo(), rx, sy+g*2, statPaint); statPaint.setTextAlign(Paint.Align.LEFT); statPaint.setColor(0xFFB0BEC5); statPaint.setAlpha(sa); statPaint.setTypeface(Typeface.DEFAULT); c.drawText("Survival Time", sx, sy+g*3, statPaint); statPaint.setTextAlign(Paint.Align.RIGHT); statPaint.setColor(WHITE); statPaint.setAlpha(va); statPaint.setTypeface(Typeface.DEFAULT_BOLD); c.drawText(w.getSurvivalTime()+"s", rx, sy+g*3, statPaint); }
    private void drawSettingsScreen(Canvas c, boolean godMode) { c.drawRect(0, 0, sw, sh, dimPaint); float cw = sw * 0.9f; float ch = sh * 0.6f; float cx = sw * 0.05f; float cy = sh * 0.2f; tempRect.set(cx, cy, cx + cw, cy + ch); cardPaint.setColor(Color.argb(220, 18, 22, 35)); cardPaint.setStyle(Paint.Style.FILL); c.drawRoundRect(tempRect, 30, 30, cardPaint); cardBorderPaint.setColor(Color.argb(80, 100, 120, 180)); cardBorderPaint.setStyle(Paint.Style.STROKE); c.drawRoundRect(tempRect, 30, 30, cardBorderPaint); accentPaint.setColor(WHITE); accentPaint.setTextSize(sw * 0.07f); accentPaint.setShader(null); c.drawText("⚙ SETTINGS", sw / 2f, cy + sh * 0.07f, accentPaint); c.drawLine(cx + 40, cy + sh * 0.09f, cx + cw - 40, cy + sh * 0.09f, dividerPaint); float rowY = cy + sh * 0.15f; float rowGap = sh * 0.09f; drawSettingsLabel(c, "DIFFICULTY", cx + 40, rowY); drawThreeWayToggle(c, cx + cw - sw*0.45f, rowY - sh*0.03f, sw*0.4f, sh*0.05f, difficultyLevel, new String[]{"EASY", "NORM", "HARD"}, new int[]{Color.rgb(60, 180, 80), Color.rgb(60, 140, 220), Color.rgb(220, 60, 50)}, 0); c.drawLine(cx + 40, rowY + sh*0.04f, cx + cw - 40, rowY + sh*0.04f, dividerPaint); rowY += rowGap; drawSettingsLabel(c, "GAME SPEED", cx + 40, rowY); drawThreeWayToggle(c, cx + cw - sw*0.45f, rowY - sh*0.03f, sw*0.4f, sh*0.05f, gameSpeedLevel, new String[]{"SLOW", "NORM", "FAST"}, new int[]{Color.rgb(100, 160, 200), Color.rgb(60, 140, 220), Color.rgb(240, 160, 30)}, 1); c.drawLine(cx + 40, rowY + sh*0.04f, cx + cw - 40, rowY + sh*0.04f, dividerPaint); rowY += rowGap; drawSettingsLabel(c, "SOUND", cx + 40, rowY); drawOnOffToggle(c, cx + cw - sw*0.25f, rowY - sh*0.03f, sw*0.2f, sh*0.045f, soundEnabled, soundToggleRect); c.drawLine(cx + 40, rowY + sh*0.04f, cx + cw - 40, rowY + sh*0.04f, dividerPaint); rowY += rowGap; drawSettingsLabel(c, "VIBRATION", cx + 40, rowY); drawOnOffToggle(c, cx + cw - sw*0.25f, rowY - sh*0.03f, sw*0.2f, sh*0.045f, vibrationEnabled, vibToggleRect); if (godMode) { smallPaint.setColor(GOLD); smallPaint.setTextSize(sw * 0.035f); c.drawText("★ GOD MODE ACTIVE ★", sw / 2f, cy + ch - 40, smallPaint); } drawCloseButton(c, sw - sw * 0.1f, sh * 0.08f); }
    ...
    private void drawCloseButton(Canvas c, float cx, float cy) { closeBtnCirclePaint.setColor(Color.argb(180, 200, 50, 50)); c.drawCircle(cx, cy, sw * 0.05f, closeBtnCirclePaint); closeBtnXPaint.setColor(WHITE); closeBtnXPaint.setStrokeWidth(5f); float cross = sw * 0.02f; c.drawLine(cx - cross, cy - cross, cx + cross, cy + cross, closeBtnXPaint); c.drawLine(cx + cross, cy - cross, cx - cross, cy + cross, closeBtnXPaint); }
    private void drawCreditDisplay(Canvas c, EconomyManager economy, float alpha) { float cx = sw * 0.05f, cy = sh * 0.07f; int credits = economy.getDisplayedCredits(); float flash = economy.getCreditFlash(); int r = (int)(200 + 55 * flash), g = (int)(180 + 75 * flash); btnTextPaint.setColor(Color.argb((int)(255 * alpha), Math.min(255, r), Math.min(255, g), 50)); btnTextPaint.setTextSize(sw * 0.06f); btnTextPaint.setTextAlign(Paint.Align.LEFT); c.drawText("✦ " + credits, cx, cy, btnTextPaint); btnTextPaint.setTextAlign(Paint.Align.CENTER); }
    private void drawInGameCredits(Canvas c, EconomyManager economy) { int credits = economy.getDisplayedCredits(); float flash = economy.getCreditFlash(); int r = (int)(200 + 55 * flash), g = (int)(180 + 75 * flash); btnTextPaint.setColor(Color.argb(180, Math.min(255, r), Math.min(255, g), 50)); btnTextPaint.setTextSize(sw * 0.045f); btnTextPaint.setTextAlign(Paint.Align.RIGHT); c.drawText("✦ " + credits, sw - sw * 0.04f, sh * 0.11f, btnTextPaint); btnTextPaint.setTextAlign(Paint.Align.CENTER); }
    ...
    public boolean handleSettingsTouch(float tx, float ty, GameWorld world) { if (isCloseHit(tx, ty)) { world.quitToMenu(); return true; } for (int i = 0; i < 3; i++) { if (settingsToggleRects[i].contains(tx, ty)) { difficultyLevel = i; saveSettings(); return true; } } for (int i = 0; i < 3; i++) { if (settingsToggleRects[3+i].contains(tx, ty)) { gameSpeedLevel = i; saveSettings(); return true; } } if (soundToggleRect.contains(tx, ty)) { soundEnabled = !soundEnabled; soundToggleCount++; if (soundToggleCount >= 10) godModeActive = true; saveSettings(); return true; } if (vibToggleRect.contains(tx, ty)) { vibrationEnabled = !vibrationEnabled; vibrationToggleCount++; if (vibrationToggleCount >= 5) godModeActive = false; saveSettings(); return true; } return false; }
