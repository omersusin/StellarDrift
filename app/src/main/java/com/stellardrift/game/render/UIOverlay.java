...
public class UIOverlay {
    ...
    private float mainBtnWidth, mainBtnHeight, cardRight;
    ...
    private void drawMenu(Canvas c, int highScore, EconomyManager economy) {
        ...
        int credits = (economy != null) ? economy.getDisplayedCredits() : 0; c.drawText("✦ " + credits, sw / 2f, sh * 0.76f, menuCreditPaint);
        menuSubPaint.setColor(Color.argb(90, 155, 165, 185)); menuSubPaint.setAlpha(90); c.drawText("Best: " + highScore, sw / 2f, sh * 0.81f, menuSubPaint);
    }

    private void drawMenuButton(Canvas c, String text, RectF outRect, int bgCol, int borderCol, int textCol) {
        menuBtnBgPaint.setColor(bgCol); menuBtnBgPaint.setStyle(Paint.Style.FILL); menuBtnBgPaint.setShader(null); c.drawRoundRect(outRect, outRect.height()/2, outRect.height()/2, menuBtnBgPaint);
        menuBtnBorderPaint.setColor(borderCol); menuBtnBorderPaint.setStyle(Paint.Style.STROKE); menuBtnBorderPaint.setStrokeWidth(3f); menuBtnBorderPaint.setShader(null); c.drawRoundRect(outRect, outRect.height()/2, outRect.height()/2, menuBtnBorderPaint);
    ...
    private void drawSettingsScreen(Canvas c, boolean godMode) {
        ...
        rowY += rowGap; drawSettingsLabel(c, "SOUND", cx + 40, rowY); drawOnOffToggle(c, cx + cw - sw*0.25f, rowY - sh*0.03f, sw*0.2f, sh*0.045f, soundEnabled, soundToggleRect); c.drawLine(cx + 40, rowY + sh*0.04f, cx + cw - 40, rowY + sh*0.04f, dividerPaint);
        rowY += rowGap; drawSettingsLabel(c, "VIBRATION", cx + 40, rowY); drawOnOffToggle(c, cx + cw - sw*0.25f, rowY - sh*0.03f, sw*0.2f, sh*0.045f, vibrationEnabled, vibToggleRect);
        if (godMode) { smallPaint.setColor(GOLD); smallPaint.setTextSize(sw * 0.035f); c.drawText("★ GOD MODE ACTIVE ★", sw / 2f, cy + ch - 40, smallPaint); }
    ...
    private void drawShop(Canvas c, ShipRegistry registry, EconomyManager economy, ShipRenderer renderer) {
        float alpha = shopOpenAnim; int blackAlpha = (int)(255 * alpha); bgBlackPaint.setColor(Color.argb(blackAlpha, 0, 0, 0)); c.drawRect(0, 0, sw, sh, bgBlackPaint); int overlayAlpha = (int)(245 * alpha); bgOverlayPaint.setColor(Color.argb(overlayAlpha, 6, 8, 16)); c.drawRect(0, 0, sw, sh, bgOverlayPaint);
        c.save(); c.translate(0, -scrollY);
        accentPaint.setTextSize(sw * 0.08f); accentPaint.setAlpha((int)(255 * alpha)); accentPaint.setShader(null); accentPaint.setColor(Color.WHITE); c.drawText("✦ HANGAR ✦", sw / 2f, sh * 0.08f + scrollY * 0.5f, accentPaint); 
        for (int i = 0; i < registry.getShipCount(); i++) { ShipData ship = registry.getShip(i); float cardY = cardStartY + i * (cardHeight + cardSpacing); if (cardY - scrollY > sh + 50 || cardY + cardHeight - scrollY < -50) continue; drawShipCard(c, ship, i, cardY, alpha, economy, renderer); }
        c.restore(); drawCloseButton(c, sw - sw * 0.1f, sh * 0.08f, alpha); drawCreditDisplay(c, economy, alpha);
    }
    ...
    private void drawMainButton(Canvas c, ShipData ship, int index, boolean isSelected, boolean isUnlocked, float al, EconomyManager economy) {
        RectF br = mainButtonRects[index]; float btnTextSize = Math.max(10, Math.min(13, mainBtnHeight * 0.4f)); textPaint.setShader(null); textPaint.setTextAlign(Paint.Align.CENTER); textPaint.setFakeBoldText(true); textPaint.setTextSize(btnTextSize); textPaint.setStyle(Paint.Style.FILL); float textY = br.centerY() + btnTextSize * 0.35f; float radius = mainBtnHeight / 2;
        if (isSelected) { buttonPaint.setColor(Color.argb((int)(170 * al), 35, 150, 70)); buttonPaint.setStyle(Paint.Style.FILL); c.drawRoundRect(br, radius, radius, buttonPaint); textPaint.setColor(Color.argb((int)(255 * al), 255, 255, 255)); c.drawText("✓ EQUIPPED", br.centerX(), textY, textPaint); } 
        else if (isUnlocked) { buttonPaint.setColor(Color.argb((int)(170 * al), 50, 110, 170)); buttonPaint.setStyle(Paint.Style.FILL); c.drawRoundRect(br, radius, radius, buttonPaint); textPaint.setColor(Color.argb((int)(255 * al), 255, 255, 255)); c.drawText("EQUIP", br.centerX(), textY, textPaint); } 
        else { boolean canAfford = economy.getCredits() >= ship.price; int btnCol = canAfford ? Color.argb((int)(190 * al), 190, 155, 30) : Color.argb((int)(90 * al), 65, 65, 72); buttonPaint.setColor(btnCol); buttonPaint.setStyle(Paint.Style.FILL); c.drawRoundRect(br, radius, radius, buttonPaint); textPaint.setColor(canAfford ? Color.argb((int)(255 * al), 255, 255, 255) : Color.argb((int)(140 * al), 160, 160, 170)); String priceText = "🔒 " + ship.price + " ✦"; if (textPaint.measureText(priceText) > br.width() - 10) textPaint.setTextSize(btnTextSize - 2); c.drawText(priceText, br.centerX(), textY, textPaint); }
        textPaint.setTextAlign(Paint.Align.LEFT); textPaint.setFakeBoldText(false);
    }
    ...
    private void drawCreditDisplay(Canvas c, EconomyManager economy, float alpha) {
        ...
        creditPaint.setColor(Color.argb((int)(255 * alpha), Math.min(255, r), Math.min(255, g), 50)); creditPaint.setTextSize(sw * 0.06f); creditPaint.setTextAlign(Paint.Align.LEFT); c.drawText("✦ " + credits, cx, cy, creditPaint); creditPaint.setTextAlign(Paint.Align.CENTER);
    }
    ...
    private void drawInGameCredits(Canvas c, EconomyManager economy) {
        ...
        creditPaint.setColor(Color.argb(180, Math.min(255, r), Math.min(255, g), 50)); creditPaint.setTextSize(sw * 0.045f); creditPaint.setTextAlign(Paint.Align.RIGHT); c.drawText("✦ " + credits, sw - sw * 0.04f, sh * 0.11f, creditPaint); creditPaint.setTextAlign(Paint.Align.CENTER);
    }
    ...
    private void drawGameOver(Canvas c, GameWorld world) {
        ...
        if (el > BUTTON_DELAY) { float ba = Math.min(1f, (el - BUTTON_DELAY) / 500f); drawMenuButton(c, "▶ PLAY AGAIN", restartBtn, Color.argb((int)(0x33*ba),255,255,255), Color.argb((int)(140*ba),0,229,255), WHITE); subtitlePaint.setAlpha((int)(80 * ba)); c.drawText("or tap anywhere for menu", cx, sh * 0.87f, subtitlePaint); }
    }
